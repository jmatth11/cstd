...
base_str_error new_base_str(struct base_str_t* data, size_t cap) {
    ...
    tmp[0] = '\0';
    data->_internal = (struct internal_base_str*)malloc(sizeof(struct internal_base_str));
...
base_str_error new_base_str_with_string(struct base_str_t* data, const char* str, size_t len) {
    if (len <= 0) {
        len = 1;
    }
