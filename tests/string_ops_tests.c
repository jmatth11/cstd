...
static bool test_str_cpy_full() {
  color_log(LOG_COLOR_CYAN, "test_str_cpy_full\n");
  const char *input = "hello";
  size_t input_len = strlen(input);
  char *result = malloc(sizeof(char) + input_len);
  if (str_cpy(result, input_len, input, input_len) == NULL) {
    color_log(LOG_COLOR_RED, "%s: str_cpy full length returned NULL\n",
              string_ops_suite_name);
    return false;
  }
  if (strcmp(input, result) != 0) {
    color_log(LOG_COLOR_RED,
              "%s: str_cpy full length should have equal strings\n",
              string_ops_suite_name);
    free(result);
    return false;
  }
...
static bool test_str_cpy_dest_len() {
  color_log(LOG_COLOR_CYAN, "test_str_cpy_full\n");
  const char *input = "hello";
  size_t input_len = strlen(input);
  size_t result_len = input_len - 2;
  char *result = malloc(sizeof(char) + result_len);
  if (str_cpy(result, result_len, input, input_len) == NULL) {
    color_log(LOG_COLOR_RED, "%s: str_cpy dest length returned NULL\n",
              string_ops_suite_name);
    return false;
  }
  if (strcmp(input, result) == 0) {
    color_log(LOG_COLOR_RED,
              "%s: str_cpy dest length should not have equal strings\n",
              string_ops_suite_name);
    free(result);
    return false;
  }
  if (strncmp(result, input, result_len) != 0) {
    color_log(LOG_COLOR_RED,
              "%s: str_cpy dest length should have equal sub strings\n",
              string_ops_suite_name);
    free(result);
    return false;
  }
...
static bool test_str_cpy_src_len() {
  color_log(LOG_COLOR_CYAN, "test_str_cpy_full\n");
  const char *input = "hello";
  size_t input_len = strlen(input);
  size_t result_len = input_len * 2;
  ...
  if (strcmp(input, result) != 0) {
    color_log(LOG_COLOR_RED,
              "%s: str_cpy src length should have equal strings\n",
              string_ops_suite_name);
    free(result);
    return false;
  }
...
static bool test_concat_empty_strings() {
  ...
  if (result == NULL) {
    color_log(LOG_COLOR_YELLOW,
              "%s: concat empty returned NULL (empty concat not supported)\n",
              string_ops_suite_name);
    return true;
  }
  if (len != 0) {
...
static bool test_to_str_length_int() {
  ...
  if (len != 4) {
    color_log(LOG_COLOR_RED,
              "%s: to_str_length_int(-456) expected 4, got %lu\n",
              string_ops_suite_name, len);
    return false;
  }
  return true;
...
static void string_ops_tests() {
  ...
  if (!test_str_cpy_full()) {
    color_log(LOG_COLOR_RED, "%s: test_str_cpy_full failed\n",
              string_ops_suite_name);
    failures++;
  }
  if (!test_str_cpy_dest_len()) {
    color_log(LOG_COLOR_RED, "%s: test_str_cpy_dest_len failed\n",
              string_ops_suite_name);
    failures++;
  }
  if (!test_str_cpy_src_len()) {
    color_log(LOG_COLOR_RED, "%s: test_str_cpy_src_len failed\n",
              string_ops_suite_name);
    failures++;
  }
  if (!test_str_fmt_int()) {
