...
/**
 * Get the range of codepoints as raw bytes at the given codepoint position and length.
 * You are responsible for freeing the returned range.
 *
 * @param[in] str The unicode string.
 * @param[in] index The codepoint position, not the byte position.
 * @param[in] len The length of the range based off the index in codepoint length.
